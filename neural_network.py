...
class NeuralNetwork:
    ...
    def load_weights_other_model(self, other_model):
        """
    ...
    def update_tgt_model_weights(self, other_model, tau):
        """
        ...
        """
        otherModelWeights = other_model.model.get_weights()
        own_weights = self.model.get_weights()
        for i in range(len(own_weights)):
            own_weights[i] = tau * otherModelWeights[i] + (1 - tau) * own_weights[i]
        self.model.set_weights(own_weights)

    def saveModel(self, checkpoint=False):
        """
        saves the model
        """
    ...
    def get_probabilistic_move(self, moves, q_values, old_board_nnet_input, temperature):
        ...
        for move_value_difference in move_value_differences:
            move_prob.append(np.exp(move_value_difference / temperature) / sum)
        random_number = random.random()
        for i in range(len(move_prob)):
            if random_number <= move_prob[i]:
                return moves[i], i
            else:
                random_number -= move_prob[i]
        # if no move selected due to overflow, return move with the highest probability
        move_index = np.argmax(move_prob)
