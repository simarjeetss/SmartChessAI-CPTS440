...
class Training:
    ...
    def __init__(self, model_name, save=True, load_checkpoint=False):
        ...
        self.training_game = ChessGame(player_1_type='ai_training', player_2_type='ai_training')
        self.training_graph = Graph(model_name=model_name, opponent="all")

    def define_filepaths(self):
        self.main_filepath = "saves/" + self.model_name + "/ChessAi_"
    ...
    def train(self):
        ...
        if self.starting_episode == 0:
            ...
            self.target_model.load_weights_other_model(self.main_model)
        else:
        ...
        for episode in range(1000000):
            ...
            while n_steps < self.n_steps_per_episode:
                ...
                while not game_done:
                    ...
                    if steps_since_last_target_model_update >= self.steps_to_update_target_model and episode > self.min_replay_size / self.n_steps_per_episode:
                        ...
                        self.target_model.update_tgt_model_weights(self.main_model, self.tau)
                if white_done:
                    self.replay_memory.append([white_best_nnet_input, white_reward, white_new_nnet_input, game_done])
                elif black_done:
                    self.replay_memory.append([black_best_nnet_input, black_reward, black_new_nnet_input, game_done])
            ...
            if self.save:
                if episode % self.n_epochs_between_saving == 0 and episode != 0:
                    ...
                    self.main_model.saveModel()
                    self.target_model.saveModel()
                ...
                if episode % self.n_epochs_between_evaluation == 0 and episode != 0:
                    evaluation_game_random = ChessGame(player_1_type='ai_evaluating', player_2_type='random', window=False,
                                                  nnets=[self.main_model, None], ai_decision_temperature=0.1)
                    evaluation_game_50_50 = ChessGame(player_1_type='ai_evaluating', player_2_type='50_minimax_50_random', window=False,
                                                 search_depth=2, nnets=[self.main_model, None], ai_decision_temperature=0.1)
                    evaluation_game_90_10 = ChessGame(player_1_type='ai_evaluating', player_2_type='90_minimax_10_random', window=False,
                                                 search_depth=2, nnets=[self.main_model, None], ai_decision_temperature=0.1)
                    evaluation_game_minimax = ChessGame(player_1_type='ai_evaluating', player_2_type='minimax', window=False,
                                                   search_depth=2, nnets=[self.main_model, None], ai_decision_temperature=0.1)
                    print("---------------")
                    # random opponent
                    print("---------------")
                    ...
                    ai_wins, random_wins, draws, board_states_of_a_game, average_game_length = evaluation_game_random.gamePlay(n_games=n_evaluation_games_random)
                    if episode % 100 == 0:
                    ...
                    ai_wins, random_wins, draws, board_states_of_a_game, average_game_length = evaluation_game_50_50.gamePlay(n_games=n_evaluation_games_50_50)
                    if episode % 100 == 0:
                    ...
                    ai_wins, random_wins, draws, board_states_of_a_game, average_game_length = evaluation_game_90_10.gamePlay(n_games=n_evaluation_games_90_10)
                    if episode % 100 == 0:
                    ...
                    ai_wins, minimax_wins, draws, board_states_of_a_game, average_game_length = evaluation_game_minimax.gamePlay(n_games=50)
                    if episode % 100 == 0:
                    ...
                    if 100 * evaluation_score / n_evaluation_games_minimax > max(self.evaluation_scores[0]) and episode >= self.checkpoint_min_epochs:
                        n_evaluation_games_minimax += 50
                        ai_wins_add, minimax_add, draws_add, _, _ = evaluation_game_minimax.gamePlay(n_games=50)
                        ...
                        if 100 * evaluation_score / n_evaluation_games_minimax > max(self.evaluation_scores[0]):
                            ...
                            self.main_model.saveModel(checkpoint=True)
                            self.target_model.saveModel(checkpoint=True)
